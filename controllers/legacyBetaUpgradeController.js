...
export const getAllLegacyBetaUpgrades = async (req, res, next) => {
  console.log('Attempting to read legacy-beta upgrades.json');
  const filePath = path.join(__dirname, '../public/converted-json/legacy-beta-upgrades/legacy-beta-upgrades.json');
  console.log('File path:', filePath);
  try {
    const data = await fs.readFile(filePath, 'utf8');
    console.log('Successfully read legacy-beta upgrades.json');
    const parsedData = JSON.parse(data);
    res.json(parsedData);
  } catch (err) {
    console.error('Error reading legacy-beta upgrades.json:', err);
    const error = new Error('Failed to read legacy-beta upgrades data');
...
export const getLegacyBetaUpgradeById = async (req, res, next) => {
  const upgradeId = req.params.id;
  console.log(`Attempting to get legacy-beta upgrade with ID: ${upgradeId}`);
  const filePath = path.join(__dirname, `../public/converted-json/legacy-beta-upgrades/${upgradeId}.json`);
  console.log(`File path for legacy-beta upgrade ${upgradeId}: ${filePath}`);
  try {
    const data = await fs.readFile(filePath, 'utf8');
    console.log(`Successfully read data for legacy-beta upgrade ${upgradeId}`);
    res.json(JSON.parse(data));
  } catch (err) {
    console.error(`Error reading legacy-beta ${upgradeId}.json:`, err);
    res.status(404).json({ message: 'Legacy-beta upgrade not found' });
  }
};

export const searchLegacyBetaUpgrades = async (req, res, next) => {
  const filePath = path.join(__dirname, '../public/converted-json/legacy-beta-upgrades/legacy-beta-upgrades.json');
  console.log('Attempting to read legacy-beta upgrades.json for search');
  try {
    const data = await fs.readFile(filePath, 'utf8');
    console.log('Successfully read legacy-beta upgrades.json for search');
    ...
    console.log(`Returning ${Object.keys(filteredUpgrades).length} legacy-beta upgrades after applying filters`);
