...
export const getAllLegacyBetaShips = async (req, res, next) => {
  console.log('Attempting to read legacy-beta ships.json');
  const filePath = path.join(__dirname, '../public/converted-json/legacy-beta-ships/legacy-beta-ships.json');
  console.log('File path:', filePath);
  try {
    const data = await fs.readFile(filePath, 'utf8');
    console.log('Successfully read legacy-beta ships.json');
    const parsedData = JSON.parse(data);
    res.json(parsedData);
  } catch (err) {
    console.error('Error reading legacy-beta ships.json:', err);
    const error = new Error('Failed to read legacy-beta ships data');
...
export const getLegacyBetaShipById = async (req, res, next) => {
  const shipId = req.params.id;
  console.log(`Attempting to get legacy-beta ship with ID: ${shipId}`);
  const filePath = path.join(__dirname, `../public/converted-json/legacy-beta-ships/${shipId}.json`);
  console.log(`File path for legacy-beta ship ${shipId}: ${filePath}`);
  try {
    const data = await fs.readFile(filePath, 'utf8');
    console.log(`Successfully read data for legacy-beta ship ${shipId}`);
    res.json(JSON.parse(data));
  } catch (err) {
    console.error(`Error reading legacy-beta ${shipId}.json:`, err);
    res.status(404).json({ message: 'Legacy-beta ship not found' });
  }
};

export const searchLegacyBetaShips = async (req, res, next) => {
  const filePath = path.join(__dirname, '../public/converted-json/legacy-beta-ships/legacy-beta-ships.json');
  console.log('Attempting to read legacy-beta ships.json for search');
  try {
    const data = await fs.readFile(filePath, 'utf8');
    console.log('Successfully read legacy-beta ships.json for search');
