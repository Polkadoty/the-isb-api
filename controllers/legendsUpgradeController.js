...
export const getAllLegendsUpgrades = async (req, res, next) => {
  ...
  try {
    ...
    console.log('Successfully read legends upgrades.json');
    const parsedData = JSON.parse(data);
    res.json(parsedData);
  } catch (err) {
    console.error('Error reading legends upgrades.json:', err);
    const error = new Error('Failed to read legends upgrades data');
...
export const getLegendsUpgradeById = async (req, res, next) => {
  ...
  try {
    const data = await fs.readFile(filePath, 'utf8');
    console.log(`Successfully read data for legends upgrade ${upgradeId}`);
    res.json(JSON.parse(data));
  } catch (err) {
    console.error(`Error reading legends ${upgradeId}.json:`, err);
    res.status(404).json({ message: 'Legends upgrade not found' });
  }
};
...
export const searchLegendsUpgrades = async (req, res, next) => {
  ...
  try {
    const data = await fs.readFile(filePath, 'utf8');
    console.log('Successfully read legends upgrades.json for search');
    ...
    console.log(`Returning ${Object.keys(filteredUpgrades).length} legends upgrades after applying filters`);
