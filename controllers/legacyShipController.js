...
export const getAllLegacyShips = async (req, res, next) => {
  console.log('Attempting to read legacy ships.json');
  const filePath = path.join(__dirname, '../public/converted-json/legacy-ships/legacy-ships.json');
  console.log('File path:', filePath);
  try {
    const data = await fs.readFile(filePath, 'utf8');
    console.log('Successfully read legacy ships.json');
    const parsedData = JSON.parse(data);
    res.json(parsedData);
  } catch (err) {
    console.error('Error reading legacy ships.json:', err);
    const error = new Error('Failed to read legacy ships data');
...
export const getLegacyShipById = async (req, res, next) => {
  const shipId = req.params.id;
  console.log(`Attempting to get legacy ship with ID: ${shipId}`);
  const filePath = path.join(__dirname, `../public/converted-json/legacy-ships/${shipId}.json`);
  console.log(`File path for legacy ship ${shipId}: ${filePath}`);
  try {
    const data = await fs.readFile(filePath, 'utf8');
    console.log(`Successfully read data for legacy ship ${shipId}`);
    res.json(JSON.parse(data));
  } catch (err) {
    console.error(`Error reading legacy ${shipId}.json:`, err);
    res.status(404).json({ message: 'Legacy ship not found' });
  }
};

export const searchLegacyShips = async (req, res, next) => {
  const filePath = path.join(__dirname, '../public/converted-json/legacy-ships/legacy-ships.json');
  console.log('Attempting to read legacy ships.json for search');
  try {
    const data = await fs.readFile(filePath, 'utf8');
    console.log('Successfully read legacy ships.json for search');
