...
export const getAllNexusShips = async (req, res, next) => {
  console.log('Attempting to read nexus ships.json');
  const filePath = path.join(__dirname, '../public/converted-json/nexus-ships/nexus-ships.json');
  console.log('File path:', filePath);
  try {
    const data = await fs.readFile(filePath, 'utf8');
    console.log('Successfully read nexus ships.json');
    const parsedData = JSON.parse(data);
    res.json(parsedData);
  } catch (err) {
    console.error('Error reading nexus ships.json:', err);
    const error = new Error('Failed to read nexus ships data');
...
export const getNexusShipById = async (req, res, next) => {
  ...
  console.log(`File path for nexus ship ${shipId}: ${filePath}`);
  try {
    const data = await fs.readFile(filePath, 'utf8');
    console.log(`Successfully read data for nexus ship ${shipId}`);
    res.json(JSON.parse(data));
  } catch (err) {
    console.error(`Error reading nexus ${shipId}.json:`, err);
    res.status(404).json({ message: 'Nexus ship not found' });
  }
};

export const searchNexusShips = async (req, res, next) => {
  const filePath = path.join(__dirname, '../public/converted-json/nexus-ships/nexus-ships.json');
  console.log('Attempting to read nexus-ships.json for search');
  try {
    const data = await fs.readFile(filePath, 'utf8');
    console.log('Successfully read nexus-ships.json for search');
