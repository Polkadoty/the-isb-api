...
export const getAllLegacyUpgrades = async (req, res, next) => {
  console.log('Attempting to read legacy upgrades.json');
  const filePath = path.join(__dirname, '../public/converted-json/legacy-upgrades/legacy-upgrades.json');
  console.log('File path:', filePath);
  try {
    const data = await fs.readFile(filePath, 'utf8');
    console.log('Successfully read legacy upgrades.json');
    const parsedData = JSON.parse(data);
    res.json(parsedData);
  } catch (err) {
    console.error('Error reading legacy upgrades.json:', err);
    const error = new Error('Failed to read legacy upgrades data');
...
export const getLegacyUpgradeById = async (req, res, next) => {
  const upgradeId = req.params.id;
  console.log(`Attempting to get legacy upgrade with ID: ${upgradeId}`);
  const filePath = path.join(__dirname, `../public/converted-json/legacy-upgrades/${upgradeId}.json`);
  console.log(`File path for legacy upgrade ${upgradeId}: ${filePath}`);
  try {
    const data = await fs.readFile(filePath, 'utf8');
    console.log(`Successfully read data for legacy upgrade ${upgradeId}`);
    res.json(JSON.parse(data));
  } catch (err) {
    console.error(`Error reading legacy ${upgradeId}.json:`, err);
    res.status(404).json({ message: 'Legacy upgrade not found' });
  }
};

export const searchLegacyUpgrades = async (req, res, next) => {
  const filePath = path.join(__dirname, '../public/converted-json/legacy-upgrades/legacy-upgrades.json');
  console.log('Attempting to read legacy upgrades.json for search');
  try {
    const data = await fs.readFile(filePath, 'utf8');
    console.log('Successfully read legacy upgrades.json for search');
    ...
    console.log(`Returning ${Object.keys(filteredUpgrades).length} legacy upgrades after applying filters`);
