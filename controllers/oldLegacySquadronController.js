...
export const getAllOldLegacySquadrons = async (req, res, next) => {
  console.log('Attempting to read old-legacy-squadrons.json');
  const filePath = path.join(__dirname, '../public/converted-json/old-legacy-squadrons/old-legacy-squadrons.json');
  console.log('File path:', filePath);
  try {
    const data = await fs.readFile(filePath, 'utf8');
    console.log('Successfully read old-legacy squadrons.json');
    const parsedData = JSON.parse(data);
    res.json(parsedData);
  } catch (err) {
    console.error('Error reading old-legacy squadrons.json:', err);
    const error = new Error('Failed to read old-legacy squadrons data');
...
export const getOldLegacySquadronById = async (req, res, next) => {
  const squadronId = req.params.id;
  console.log(`Attempting to get old-legacy squadron with ID: ${squadronId}`);
  const filePath = path.join(__dirname, `../public/converted-json/old-legacy-squadrons/${squadronId}.json`);
  console.log(`File path for old-legacy squadron ${squadronId}: ${filePath}`);
  try {
    const data = await fs.readFile(filePath, 'utf8');
    const squadronsData = JSON.parse(data);
    const squadron = squadronsData.squadrons[squadronId];
    if (squadron) {
      console.log(`Successfully read data for old-legacy squadron ${squadronId}`);
      res.json(squadron);
    } else {
...
export const searchOldLegacySquadrons = async (req, res, next) => {
  const filePath = path.join(__dirname, '../public/converted-json/old-legacy-squadrons/old-legacy-squadrons.json');
  console.log('Attempting to read old-legacy squadrons.json for search');
  try {
    const data = await fs.readFile(filePath, 'utf8');
    console.log('Successfully read old-legacy squadrons.json for search');
    let squadronsData = JSON.parse(data);
    const filters = req.query;
    console.log('Applying filters:', filters);

    const compareValues = (value, filterValue, operator = '=') => {
      // Implementation remains the same as in old-legacySquadronController.js
    ...
    console.log(`Returning ${Object.keys(filteredSquadrons).length} old-legacy squadrons after applying filters`);
